...
GO
CREATE NONCLUSTERED INDEX [ix_AccountCode_Includes]
    ON [dbo].[JournalEntries]([AccountCode] ASC)
    INCLUDE([BPAccountCode], [CountryCode], [PostingDate], [SystemCreditAmount], [SystemDebitAmount]);


GO
CREATE NONCLUSTERED INDEX [ix_OffsetAccount_Includes]
    ON [dbo].[JournalEntries]([OffsetAccount] ASC)
    INCLUDE([BPAccountCode], [CountryCode], [PostingDate], [SystemCreditAmount], [SystemDebitAmount]);
