/*
Updates:
	V4 DK 2020-02-27: Added Logic to LineTotal field for Bin movement amounts
	V3 DK 2019-10-22: Update Att. 2 to use the correct file extension and target path field
...
select 
	...
	,Case 
		...
		When InventoryLogs.Quantity <> 0 and InventoryLogs.ActionType not in (19,20) and (
		...
		when inventorylogs.quantity <>0 and InventoryLogs.ActionType = 19 then cast((inventorylogs.price * InventoryLogs.Quantity) as float) --added this logic for Bin movements
		when inventorylogs.quantity <>0 and InventoryLogs.ActionType =20 then cast((inventorylogs.price *- InventoryLogs.Quantity) as float) --added this logic for Bin movements
	...
	,Case 
		...
		when inventorylogs.quantity <>0 and InventoryLogs.ActionType = 19 then cast((inventorylogs.price * InventoryLogs.Quantity)/ ExchangeRates.Rate as float)
		when inventorylogs.quantity <>0 and InventoryLogs.ActionType =20 then cast((inventorylogs.price * -InventoryLogs.Quantity)/ ExchangeRates.Rate as float)
